...
import { AllProduct } from '@/data/product';
  ...
  export default function Shop() {
    ...
    const[selected,setSelected]=React.useState("All Product")
  ...
  const filterProduct=selected==="All Product"?AllProduct:AllProduct.filter((item)=>item.seller===selected)
    return (
      <Box sx={{ background: "#f5f2f2", py: 5,width:"100%",color:"#423c39"}}>
          <Container maxWidth={false} disableGutters sx={{display:"flex",justifyContent:"flex-start",flexDirection:{md:"row",xs:"column"},gap:2}}>
          ...
          <Typography sx={{mt:1,textDecoration:"underline",cursor:"pointer"}} onClick={()=>setSelected("All Product")}>All Products</Typography>
          <Typography sx={{mt:1,cursor:"pointer","&:hover": {
      textDecoration: "underline",
    },}} onClick={()=>setSelected("Best Seller")}>Best Sellers</Typography>
          <Typography sx={{mt:1,cursor:"pointer","&:hover": {
      textDecoration: "underline",
    },}} onClick={()=>setSelected("Sale")}>Sale</Typography>
